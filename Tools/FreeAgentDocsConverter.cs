...
#:package ReverseMarkdown@4.7.1
#:package Spectre.Console@0.54.0
#:package Spectre.IO@0.21.0

// dotnet run Tools/FreeAgentDocsConverter.cs -- https://dev.freeagent.com/docs/invoices -o ./docs
...
{
    AnsiConsole.MarkupLine("[red]Error:[/] URL must be a valid absolute URL");
    return 1;
}

if (uri.Host != "dev.freeagent.com")
{
    AnsiConsole.MarkupLine("[red]Error:[/] URL must be from dev.freeagent.com");
    return 1;
}

if (!uri.AbsolutePath.StartsWith("/docs/"))
{
    AnsiConsole.MarkupLine("[red]Error:[/] URL must be a documentation page (e.g., /docs/attachments)");
    return 1;
}
...
void ShowUsage()
{
    AnsiConsole.MarkupLine("Usage: [blue]FreeAgentDocsConverter.cs[/] [yellow]<url>[/] [[options]]");
    AnsiConsole.WriteLine();
    AnsiConsole.MarkupLine("Arguments:");
    AnsiConsole.MarkupLine("  [yellow]<url>[/]  The FreeAgent API documentation URL to convert");
    AnsiConsole.WriteLine();
    AnsiConsole.MarkupLine("Options:");
    AnsiConsole.MarkupLine("  [yellow]-o, --output <dir>[/]  Output directory (defaults to current directory)");
    AnsiConsole.WriteLine();
    AnsiConsole.MarkupLine("Examples:");
    AnsiConsole.MarkupLine("  dotnet run FreeAgentDocsConverter.cs -- https://dev.freeagent.com/docs/attachments");
    AnsiConsole.MarkupLine("  dotnet run FreeAgentDocsConverter.cs -- https://dev.freeagent.com/docs/invoices -o ./docs");
}
...
void PreProcessTableCells(HtmlNode bodyNode)
{
...
void PreProcessColspanRows(HtmlNode bodyNode)
{
...
void RemoveNodes(HtmlNode parent, string xpath)
{
...
string PostProcessLinks(string markdown)
{
    ...
    return Regex.Replace(markdown, @"\]\(/docs/([^#)]+)(#[^)]+)?\)", "](${1}.md${2})");
}

string PostProcessMarkdown(string markdown)
{
...
string FixTableColumnCounts(string markdown)
{
...
int CountTableColumns(string line)
{
...
string TryFixMissingColumn(string line, int expectedColumns, HashSet<string> knownTypes)
{
...
List<string> FormatSingleTable(List<string> tableLines)
{
